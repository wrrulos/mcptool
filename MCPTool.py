# =============================================================================
#                      MCPTool v2.2 www.github.com/wrrulos
...
help_message = f"""
     ...
     ║  bot [ip:port]               ║  Check if the server can be entered   ║
     ║                              ║  using a bot                          ║ 
...
def connect(host, port):
    try:
        os.remove("config/files/connect.txt")

    except:
        pass

    os.system(f"cd config/files && python connect.py -host {host} -p {port}")
    check__encoding = check_encoding("config/files/connect.py")
    f = open("config/files/connect.txt", "r+", encoding=check__encoding)
    content = f.read()
...
def replace_text(text):
    """ Replace the text """
    if 'translate:multiplayer.disconnect.not_whitelisted' in text:
        text = text.replace('translate:multiplayer.disconnect.not_whitelisted', "§bWhitelist")

    if 'translate:multiplayer.disconnect.unverified_username' in text:
        text = text.replace('translate:multiplayer.disconnect.unverified_username', "§ePremium Server")

    if "translate:multiplayer.disconnect.banned.reason,with:[" in text:
        text = text.replace("translate:multiplayer.disconnect.banned.reason,with:[", "§cBanned: §f")

    if "translate:multiplayer.disconnect.banned_ip.reason,with:[" in text:
        text = text.replace("translate:multiplayer.disconnect.banned_ip.reason,with:[", "§cIP Banned: §f")

    if "extra:[text:Not authenticated with ,clickEvent:action:open_url,value:http://Minecraft.net,text:Minecraft.net,text:],text:" in text:
        text = text.replace('extra:[text:Not authenticated with ,clickEvent:action:open_url,value:http://Minecraft.net,text:Minecraft.net,text:],text:', "§eServer premium or bot name is premium and server is semi premium")

    if "extra:[bold:false,italic:false,underlined:false,strikethrough:false,obfuscated:false,color:red," in text:
        text = text.replace("extra:[bold:false,italic:false,underlined:false,strikethrough:false,obfuscated:false,color:red,", "§c")
    ...
    if text == "MCPTool-OK":
        text = text.replace("MCPTool-OK", "§aConnected")

    if text == "MCPTool-TIMEOUT":
        text = text.replace("MCPTool-TIMEOUT", "§cTimeout")
...
def get_server(ip, skip, logs_file):
    ...
    try:
        ...
        if bot_connect:
            host = ip.split(":")
            content = connect(host[0], host[1])
            print(f"     {lblack}[{lred}CONN{white}ECT{lblack}] {content}")
            ...
            try:
                ...
                if check__node:
                    ...
                    if check__server:
                        ...
                        content = connect(target, get__port)
                        print(f"\n     {lblack}[{lred}CONN{white}ECT{lblack}] {content}")
