...
def pinlogin_command(server, username, version, password_file, proxy_file=None):
    """
    Run the pin_login.js script to bruteforce the server
    pin authentication plugin (for example, AdvancedLogin).
    ...
    try:
        ...
        if proxy_file is not None:
            subprocess.run(f'{config_manager.config["commands"]["nodejs"]} utils/scripts/pin_login.js {server[0]} {server[1]} {username} {version} {password_file} {len(get_spaces())} {proxy_file}', shell=True)

        else:
            subprocess.run(f'{config_manager.config["commands"]["nodejs"]} utils/scripts/pin_login.js {server[0]} {server[1]} {username} {version} {password_file} {len(get_spaces())}', shell=True)
