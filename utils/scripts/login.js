...
if (passwordFile === null) {
    utils.coloredText('\n    §f[§c#§f] §fThe entered password file is not valid.');
    process.exit(1);
}
...
function createBot(proxy=null) {
    ...
    try {
        ...
        if (error.message.includes('is not supported')) {
            ...
            utils.coloredText(`\n${getSpaces(spaces)}§f[§c#§f] §4Version §c${version} §4is not currently supported.`);
        }

        else if (error.message.includes('unsupported protocol version:')) {
            const protocol = error.message.match(/\d+/)[0];
            utils.coloredText(`\n${getSpaces(spaces)}§f[§c#§f] §4Protocol §c${protocol} §4is not supported`);
        }

        else {
            utils.coloredText(`\n${getSpaces(spaces)}§f[§c#§f] §4Error`);
        }
...
function sendBot() {
    if (proxyFile) {
        ...
        if (randomProxy) {
            ...
            utils.coloredText(`\n${getSpaces(spaces)}§f§l[§c§l#§f§l] Invalid Proxy: ${_}`);
        }
