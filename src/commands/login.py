...
def login_command(server, username, version, password_file, *args):
    ...
    try:
        ...
        if JsonManager.get(["minecraftServerOptions", "proxy"]):
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/login.js {ip} {port} {username} {version} {password_file} {len(GetUtilities.get_spaces())} {JsonManager.get(["minecraftServerOptions", "proxyFileForTheBot"])}'

        else:
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/login.js {ip} {port} {username} {version} {password_file} {len(GetUtilities.get_spaces())}'
