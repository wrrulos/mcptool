...
def login_command(server, username, version, password_file, *args):
    """
    ...
    try:
        ...
        if len(passwords) == 0:
            ...
            paint(f'\n{GetUtilities.get_spaces()}{GetUtilities.get_translated_text(["prefix"])}{GetUtilities.get_translated_text(["commands", "login", "emptyFile"])}')
            return
        
        if JsonManager.get(["minecraftServerOptions", "proxy"]):
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./mcptool_files/scripts/login.js {ip} {port} {username} {version} {password_file} {len(GetUtilities.get_spaces())} {JsonManager.get(["minecraftServerOptions", "proxyFileForTheBot"])}'

        else:
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./mcptool_files/scripts/login.js {ip} {port} {username} {version} {password_file} {len(GetUtilities.get_spaces())}'
        
        # Run the connection script to connect to the server with the specified username and passwords.
