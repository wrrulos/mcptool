...
def connect_command(server, username, version, *args):
    ...
    try:
        ...
        if JsonManager.get(["minecraftServerOptions", "proxy"]):
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/connect.js {ip} {port} {username} {version} {len(GetUtilities.get_spaces())} {JsonManager.get(["minecraftServerOptions", "proxyFileForTheBot"])}'

        else:
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/connect.js {ip} {port} {username} {version} {len(GetUtilities.get_spaces())}'
