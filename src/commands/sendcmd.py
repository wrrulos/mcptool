...
def sendcmd_command(server, username, version, command_file, loop, *args):
    ...
    try:
        ...
        if JsonManager.get(["minecraftServerOptions", "proxy"]):
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/sendcmd.js {ip} {port} {username} {version} {command_file} {len(GetUtilities.get_spaces())} {JsonManager.get(["minecraftServerOptions", "proxyFileForTheBot"])}'

        else:
            command = f'{JsonManager.get(["minecraftServerOptions", "nodeCommand"])} ./src/scripts/sendcmd.js {ip} {port} {username} {version} {command_file} {len(GetUtilities.get_spaces())}'
