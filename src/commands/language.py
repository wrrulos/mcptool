...
def language_command(language, *args):
    ...
    with open(f'./mcptool_files/config/lang/{language}.json', 'r', encoding='utf8') as f:
    ...
    settings = JsonManager.load_json('./mcptool_files/config/config.json')
    settings['lang'] = language
    JsonManager.save_json(settings, './mcptool_files/config/config.json')
