...
class FakeProxy:
    ...
    def setup(address):
        ...
        if server_data['favicon'] is not None:
            with open(f'./src/proxy/jar/fakeproxy/server-icon.png', 'wb') as f:
                f.truncate(0)
                icon = str(server_data['favicon']).replace('data:image/png;base64,', '')
                icon = base64.b64decode(icon)
                f.write(icon)
        else:
            shutil.copy('./src/proxy/server-icon.png', f'./src/proxy/jar/fakeproxy/server-icon.png')

        # Write the command prefix to the Fakeproxy config.
        FileUtilities.write_file(f'./src/proxy/jar/fakeproxy/plugins/RPoisoner/config/commandPrefix', JsonManager.get(['proxyConfig', 'fakeProxyCommandPrefix']), 'w+', True)
        ...
        if os.path.exists('./src/proxy/jar/fakeproxy/plugins/RPoisoner/data.txt'):
            FileUtilities.write_file(f'./src/proxy/jar/fakeproxy/plugins/RPoisoner/config/commandPrefix', '', 'w+', True)
    ...
    def show_data(address, log_file):
        ...
        fakeproxy_data_file = './src/proxy/jar/fakeproxy/plugins/RPoisoner/data.txt'
        data_file_lines = 0
    ...
    def update_data(address):
        ...
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/onlinePlayers', str(server_data['connected_players']), 'w+', True)
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/maximumPlayers', str(server_data['max_player_limit']), 'w+', True)
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/protocol', str(server_data['protocol']), 'w+', True)
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/version', str(minimessage_colors(server_data['version'])), 'w+', True)
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/motd', str(minimessage_colors(server_data['motd'])), 'w+', True)
        FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/samplePlayers', '', 'w+', True)
        ...
        if server_data['default_player_list'] is not None:
            for player in server_data['default_player_list']:
                if type(player) == dict:
                    ...
                    FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/samplePlayers', f'{username}/#-#/{uuid}\n', 'a', False)

                else:
                    FileUtilities.write_file('./src/proxy/jar/fakeproxy/plugins/RPoisoner/settings/samplePlayers', f'{player}/#-#/00000000-0000-0000-0000-000000000000\n', 'a', False)