...
if (process.argv.length < 4) {
    console.log('Usage: node checker.js <host> <port> <version> <spaces> [<proxyFile>]');
    process.exit(1);
}
...
function createBot(proxy=null) {
    ...
    bot.on('error', (_) => {
        if (_.field === 'login.toServer') {
            console.log('§cIncompatible Minecraft version');
        } else {
